...
bool Pipeline::init(MVkRenderData &renderData, std::string vertexShaderFilename, std::string fragmentShaderFilename)
{
  /* pipeline layout */
  ...
  mainBinding.stride = sizeof(MVkVertex);
  ...
  positionAttribute.offset = offsetof(MVkVertex, position);

  VkVertexInputAttributeDescription uvAttribute{};
  uvAttribute.binding = 0;
  uvAttribute.location = 1;
  uvAttribute.format = VK_FORMAT_R32G32_SFLOAT;
  uvAttribute.offset = offsetof(MVkVertex, uv);
...
void Pipeline::cleanup(MVkRenderData &renderData)
{
