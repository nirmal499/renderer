...
MVkRenderer::MVkRenderer(GLFWwindow *window)
{
  mWindow = window;
}

bool MVkRenderer::init(unsigned int width, unsigned int height)
{
  if (!mWindow)
  {
...
bool MVkRenderer::getQueue()
{
  ...
  /* Got our present queue index */
  mRenderData.rdPresentQueue = presentQueueRet.value();

  return true;
}

bool MVkRenderer::createDepthBuffer() 
{
...
bool MVkRenderer::createSwapchain()
{
...
bool MVkRenderer::recreateSwapchain()
{
  /* handle minimize */
  ...
  /* CPU gets blocked till all the GPU works are completed */
...
bool MVkRenderer::createRenderPass()
{
...
bool MVkRenderer::createPipeline()
{
  std::string vertexShaderFile = SHADER_PATH "basic.vert.spv";
  std::string fragmentShaderFile = SHADER_PATH "basic.frag.spv";
...
bool MVkRenderer::createFramebuffer()
{
...
bool MVkRenderer::createCommandPool()
{
...
bool MVkRenderer::createCommandBuffer()
{
...
bool MVkRenderer::createSyncObjects()
{
...
bool MVkRenderer::loadTexture()
{
  std::string textureFileName = TEXTURE_PATH "pic1.jpg";
...
void MVkRenderer::setSize(unsigned int width, unsigned int height)
{
  /* Vulkan detects changes and recreates swapchain */
  Logger::log(1, "%s: resized window to %ix%i\n", __FUNCTION__, width, height);
}

bool MVkRenderer::uploadData(MVkMesh vertexData)
{
  /* needs to be split in buffer alloc and upload */

  VkBufferCreateInfo bufferInfo{};
  bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
  bufferInfo.size = vertexData.vertices.size() * sizeof(MVkVertex);
  ...
  std::memcpy(data, vertexData.vertices.data(), vertexData.vertices.size() * sizeof(MVkVertex));
